    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = (sin(p.x * 4.0 + uTime) * 0.3 + cos(p.y * 2.0 + uTime) * 0.3) * (0.05 + uSpeed * 0.3);
...
class App {
  ...
  onTouchUp() {
    this.isDown = false
    setTimeout(() => { this.isDragging = false }, 800)
    this.onCheck()
  }
  onWheel(e) {
  ...
  update() {
    if (!this.isDragging && this.autoPlay) {
      this.scroll.target += this.autoPlay * 0.016
    }
  ...
  addEventListeners() {
    ...
    // wheel only fires when hovered (passive: false to allow preventDefault)
